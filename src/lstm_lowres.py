...
5. Modified to load data from a specified parquet file and use a specific list
   of features, including 'hour_cos' and 'isNight', without additional data augmentation.
6. Added function to plot actual vs predicted power for the entire test set.
7. Modified plotting functions to keep plots open after saving.
"""
...
# This is included for consistency with the high-resolution model, although
# the original low-resolution model didn't have it. Keeping it allows for
# potential future use or alignment.
...
class LSTMLowResForecaster:
    def __init__(self, sequence_length=24, batch_size=32, epochs=50):
        ...
        self.config = {
            'lstm_units': [64, 32],  # Default 3 LSTM layers
    ...
    def load_data(self, data_path):
        ...
        This method is renamed from load_and_prepare_data for consistency
        with the high-resolution script structure.
    ...
    def objective(self, trial, X_train_seq, y_train_seq, X_val_seq, y_val_seq):
        ...
        try:
            history = model.fit(
                ...
                epochs=10,  # Reduced epochs for hyperparameter search
                batch_size=self.batch_size,
                validation_data=(X_val_seq, y_val_seq),
                callbacks=callbacks,
                verbose=0  # Silent training during hyperparameter search
            )
            # Return the best validation loss during the trial
    ...
    def run_pipeline(self, data_path, optimize_hyperparams=True, n_trials=50):
        ...
        data = self.split_and_scale_data(df_processed, feature_info)
        ...
        print(f"\nCreating sequences with sequence length: {self.sequence_length} steps ({self.sequence_length*1:.2f} hours lookback)") # Adjusted print for 1-hour data
...
if __name__ == "__main__":
    ...
    # Corrected data resolution printout for 1-hour data
    ...
    # Run pipeline
    metrics = forecaster.run_pipeline(data_path=DATA_PATH, optimize_hyperparams=OPTIMIZE, n_trials=N_TRIALS)

    # Print final results
    print("\n--- Final Results (Test Set) ---")
    # Print post-processed metrics if available, otherwise raw metrics (original then scaled fallback)
    if 'rmse_postprocessed' in metrics:
        print(f"RMSE (Post-processed): {metrics['rmse_postprocessed']:.2f}")
        print(f"MAE (Post-processed): {metrics['mae_postprocessed']:.2f}")
        print(f"MAPE (Post-processed): {metrics['mape_postprocessed']:.2f}%")
        print(f"SMAPE (Post-processed): {metrics['smape_postprocessed']:.2f}%")
        print(f"R² (Post-processed): {metrics['r2_postprocessed']:.4f}")
    elif 'rmse_raw' in metrics: # Print raw metrics if calculated
         print(f"RMSE (Raw): {metrics['rmse_raw']:.2f}")
         print(f"MAE (Raw): {metrics['mae_raw']:.2f}")
         print(f"MAPE (Raw): {metrics['mape_raw']:.2f}%")
         print(f"SMAPE (Raw): {metrics['smape_raw']:.2f}%")
         print(f"R² (Raw): {metrics['r2_raw']:.4f}")
    else: # Fallback to scaled raw metrics if nothing else
         print(f"RMSE (Scaled Raw Fallback): {metrics['rmse_scaled']:.6f}")
         print(f"MAE (Scaled Raw Fallback): {metrics['mae_scaled']:.6f}")
         print(f"MAPE (Scaled Raw Fallback): {metrics['mape_scaled']:.2f}%")
         print(f"SMAPE (Scaled Raw Fallback): {metrics['smape_scaled']:.2f}%")
         print(f"R² (Scaled Raw Fallback): {metrics['r2_scaled']:.4f}")
    print("-" * 35)
